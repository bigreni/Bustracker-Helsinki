(function() {
  ...
  mqtt_to_live = function(topic, data) {
    var dir, geohash, headsign, info, line, mode, next_stop, start_time, vehi, _, _ref;
    _ref = topic.split('/'), _ = _ref[0], _ = _ref[1], _ = _ref[2], mode = _ref[3], vehi = _ref[4], line = _ref[5], dir = _ref[6], headsign = _ref[7], start_time = _ref[8], next_stop = _ref[9], geohash = 11 <= _ref.length ? __slice.call(_ref, 10) : [];
    info = data.VP;
    return {
      vehicle: {
        id: vehi
  ...
  interpret_jore = function(routeId) {
    ...
    if (citynavi.config.id !== "helsinki") {
      ...
      _ref4 = ["TRAM", 0, routeId.replace(/^.0*/, "")], mode = _ref4[0], routeType = _ref4[1], route = _ref4[2];
    } else if (routeId != null ? routeId.match(/^(1|2|4|5|6|7|9).../) : void 0) {
      _ref5 = ["BUS", 3, routeId.replace(/^.0*/, "")], mode = _ref5[0], routeType = _ref5[1], route = _ref5[2];
    } else {
  ...
  $('#live-page').bind('pageshow', function(e, data) {
    ...
    return $.getJSON("https://dev.hsl.fi/vehicle/journey/", function(data) {
      var body, count, error, mqtt, sub, topic, _ref;
      count = 0;
      for (topic in data) {
        body = data[topic];
        handle_vehicle_update(true, mqtt_to_live(topic, body));
        count = count + 1;
      }
      console.log("Got " + count + " vehicles in " + citynavi.config.name);
      try {
        if ($.browser.mozilla && Number($.browser.version) < 30) {
          foo.bar.baz.fail();
        }
        mqtt = new Paho.MQTT.Client("mqtt.hsl.fi", 443, "", "");
        mqtt.connect({
          useSSL: true,
          onSuccess: function() {
            return mqtt.subscribe("/hfp/journey/#");
          },
          timeout: 5,
          onFailure: function() {
            var sub, _ref;
            return sub = (_ref = citynavi.realtime) != null ? _ref.client.subscribe("/location/" + citynavi.config.id + "/**", function(msg) {
              return handle_vehicle_update(false, msg);
            }) : void 0;
          }
        });
        mqtt.onMessageArrived = function(message) {
          return handle_vehicle_update(false, mqtt_to_live(message.destinationName, JSON.parse(message.payloadString)));
        };
      } catch (_error) {
        error = _error;
        sub = (_ref = citynavi.realtime) != null ? _ref.client.subscribe("/location/" + citynavi.config.id + "/**", function(msg) {
          return handle_vehicle_update(false, msg);
        }) : void 0;
      }
      return $('#live-page [data-rel="back"]').on('click', function(e) {
        return sub.cancel();
      });
    });
  });
  ...
  $('#live-page [data-rel="back"]').on('click', function(e) {
    return reset_map();
  });
  ...
  render_route_layer = function(itinerary, routeLayer) {
    var leg, legs, route_id, route_includes_transit, sum, total_walking_distance, total_walking_duration, _i, _len, _ref1, _results;
    legs = itinerary.legs;
    for (route_id in ((_ref1 = citynavi.realtime) != null ? _ref1.subs : void 0) || []) {
    ...
    for (_i = 0, _len = legs.length; _i < _len; _i++) {
      ...
      _results.push((function(leg) {
        var color, dashArray, icon, label, last_stop, marker, point, points, polyline, secondsCounter, stop, uid, _ref2, _ref3;
        uid = Math.floor(Math.random() * 1000000);
        points = (function() {
          var _j, _len1, _ref2, _results1;
          _ref2 = leg.legGeometry.points;
          _results1 = [];
          for (_j = 0, _len1 = _ref2.length; _j < _len1; _j++) {
            point = _ref2[_j];
        ...
        color = google_colors[(_ref2 = leg.routeType) != null ? _ref2 : leg.mode];
        ...
        if (true) {
          ...
          label = "<span style='font-size: 24px;'><img src='static/images/" + google_icons[(_ref3 = leg.routeType) != null ? _ref3 : leg.mode] + "' style='vertical-align: sub; height: 24px'/><span>" + leg.route + "</span></span>";
          secondsCounter = function() {
            var duration, hours, minutes, now, real_time, seconds, sign;
            now = citynavi.time();
        ...
        if (leg.routeType != null) {
          $.getJSON(citynavi.config.otp_base_url + "transit/variantForTrip", {
            ...
            points = (function() {
              ...
              _ref4 = decode_polyline(geometry.points, 2);
              _results1 = [];
          ...
          $.getJSON(citynavi.config.siri_url, {
            ...
            var vehicle, _j, _len1, _ref4, _ref5;
            _ref4 = data.Siri.ServiceDelivery.VehicleMonitoringDelivery[0].VehicleActivity;
            for (_j = 0, _len1 = _ref4.length; _j < _len1; _j++) {
              vehicle = _ref4[_j];
              handle_vehicle_update(true, siri_to_live(vehicle));
            }
            ...
            return (_ref5 = citynavi.realtime) != null ? _ref5.subscribe_route(leg.routeId, function(msg) {
              return handle_vehicle_update(false, msg);
            }) : void 0;
          });
        }
  ...
  handle_vehicle_update = function(initial, msg) {
    var direction, icon, id, interpolation, itinerary, leg, mode, old_pos, pos, route, routeType, start_time, steps, _i, _j, _len, _ref1, _ref2, _ref3;
    id = msg.vehicle.id;
    ...
    if (!(id in vehicles)) {
      icon = L.divIcon({
        className: "navigator-div-icon",
        html: "<div id='vehicle-" + id + "' style='background: " + google_colors[routeType != null ? routeType : mode] + "'><span>" + route + "</span></div>"
      });
    ...
    if (itinerary != null) {
      ...
      for (_i = 0, _len = _ref2.length; _i < _len; _i++) {
        leg = _ref2[_i];
        if (leg.routeType != null) {
          _ref3 = leg.tripId.split('_'), route = _ref3[0], _j = _ref3.length - 2, direction = _ref3[_j++], start_time = _ref3[_j++];
          if (route === msg.trip.route && direction === msg.trip.direction && start_time === msg.trip.start_time) {
            $("#vehicle-" + id).css('box-shadow', "0px 0px 10px 10px " + google_colors[routeType != null ? routeType : mode]);
            leg.realTime = true;
          ...
          } else if (route === msg.trip.route && direction !== msg.trip.direction) {
    ...
    return $("#vehicle-" + id + " span").css("transform", "rotate(" + ((msg.position.bearing + 90) * -1) + "deg)");
  };
  ...
  map.on('locationfound', function(e) {
    var bbox_ne, bbox_sw, measure, point, radius, zoom;
    radius = e.accuracy;
  ...
  $('#navigation-page [data-rel="back"]').on('click', function(e) {
